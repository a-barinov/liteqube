...
add_line dom0 "/etc/qubes-rpc/policy/admin.vm.feature.CheckWithTemplate" "${VM_AUDIO} "'$adminvm allow,target=$adminvm'
...
for VM in ${QUBES_WITH_SOUND} ; do
    ...
    add_line dom0 "/etc/qubes-rpc/policy/admin.vm.feature.CheckWithTemplate" "${VM_AUDIO} ${VM} "'allow,target=$adminvm'
